...
int32_t dump_data(unsigned long phys_addr,uint32_t size,uint16_t optype,uint32_t memid, uint32_t offset)
{
       ...
       // write_config_file(reserve_virt_addr,size);
        ...
        sprintf(path, "optype_%d_memid_%d_size_%d_offset_%d.bin",optype,memid, size,offset);
    ...
    set_fs(KERNEL_DS);
    pos =0;
    vfs_write(file, (void *)reserve_virt_addr, size, &pos);
