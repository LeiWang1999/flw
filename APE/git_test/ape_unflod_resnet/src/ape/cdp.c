...
static ScU32
map_local_size(ScU8 local_size)
{
...
void cdp_parse(void * data) {
	...
	cdp_surface = (	struct cdp_surface_desc *)cdp_data->surface_addr;
	cdp_op = (struct cdp_op_desc *)cdp_data->op_addr;

	ape_debug_cdp_surface_desc(cdp_surface, 1);
	ape_debug_cdp_op_desc(cdp_op, 1);
}
